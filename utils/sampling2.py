...
def cifar_longtail(dataset, num_users, ir):
    """
    ...
    # print("length of list_label2indices = ", len(list_label2indices))

    # Get num_list for each class
    # Long tailed distribution with exp decay
    img_max = len(dataset) / num_classes
    img_num_list = []
    for _classes_idx in range(num_classes):
        nn = img_max * (ir ** (_classes_idx / (num_classes - 1.0)))
        img_num_list.append(int(nn))
    ...
    for _class, _img_num in zip(classes, img_num_list):
        ...
        # reduce data items for _class
        idx = indices[:_img_num]
        class_indices.extend(idx)
    # print("len(class_indices) = ", len(class_indices))

    # Assign each client with non-iid long-tailed data
    num_items = int(sum(img_num_list) / num_users)
    dict_users, all_idxs = {}, [i for i in range(len(class_indices))]
    for i in range(num_users):
        dict_users[i] = set(all_idxs[:num_items])
        all_idxs = list(set(all_idxs) - dict_users[i])
    print("user_data : ", len(dict_users), len(dict_users[0]))
    return dict_users
